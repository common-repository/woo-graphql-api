...
abstract class MobileDriver
{
    ...
    public function sendOTP($to, $options)
    {
        ...
        if ($options['purpose'] == "register") {
            $user = WC()->GQL_User->getUserByMobile($telephone);
        ...
        $text = $this->getUniquePrefix() . "\n" . str_replace("SHOPZOTPCODE", $code, $message_template);
    ...
    public function sendForgetPassword($to, $options)
    {
        ...
        $text = $this->getUniquePrefix() . "\n" . str_replace("SHOPZFORGOT", $forgotPassURL, $message_template);
